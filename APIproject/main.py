...
@app.get("/weather")
def get_weather(
    latitude: float = 51.5002, longitude: float = -0.120000124, options: str = ""
):
    log.info(options)
    log.info(f"Requested latitude: {latitude} and longitude: {longitude}")
    output = weather_api.get_weather(
        longitude=longitude, latitude=latitude, options=options
    )
...
def chart_output(
    ...
    return templates.TemplateResponse(
        ...
        {
            "request": request,
            "data": [data, labels, values, labelsvaluespair],
